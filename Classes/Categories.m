...
@synthesize viewBGColored;
@synthesize imgMovable;
...
//	[[UIApplication sharedApplication] setStatusBarHidden:NO];
    ...
    // set the image
    //[self.navigationController.navigationBar setTintColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"topbar_logo.png"]]];

  //  self.navigationController.navigationBar.autoresizesSubviews=YES;
	//self.navigationItem.titleView.center = CGPointMake(160, 50);
 	//[self menuBar];
	    //[self.viewBtnBack setBackgroundImage:[UIImage imageNamed:@"Nav_accomm_on.png"] forState:UIControlStateSelected];	
    ...
    viewBGColored.backgroundColor = [UIColor colorWithRed:0/255.f green:152/255.f blue:179/255.f alpha:1.0]; 
...
-  (void) menuBar{
	...
	sport.frame = CGRectMake(253, 4, 64, 59);
...
-(void)viewWillAppear:(BOOL)animated{
  ...
  /*  UIImageView *navigationBarView = [[UIImageView alloc] 
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"topbar_default_logo.png"] forBarMetrics:UIBarMetricsDefault];
    [self.navigationController.navigationBar setTintColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"topbar_default_logo.png"]]];
    ...
    */
    if (bShowMenuBar == NO) {
        ...
        self.tableView.frame = CGRectMake(0, 113-70, 320, 255 + 20);
...
- (void)Arts:(UIButton *)sender {
    ...
    arrow.image = arw;
     */
    imgMovable.frame = CGRectMake(-190, 0, 575, 72);
        
    [UIView commitAnimations];

    appDelegate.strMenuSelected = @"ARTS";
    ...
    viewBGColored.backgroundColor = [UIColor colorWithRed:186/255.f green:39/255.f blue:91/255.f alpha:1.0];
...
- (void)Community:(UIButton *)sender {
	...
	arrow.image = arw;
     */
    imgMovable.frame = CGRectMake(-125, 0, 575, 72);
    
    [UIView commitAnimations];
    
    appDelegate.strMenuSelected = @"COMMUNITY";
	showAlert = YES;
    ...
    viewBGColored.backgroundColor = [UIColor colorWithRed:242/255.f green:96/255.f blue:0/255.f alpha:1.0]; 
...
- (void)Sport:(UIButton *)sender {
	...
	arrow.image = arw;
     */
    imgMovable.frame = CGRectMake(0, 0, 575, 72);
    
    [UIView commitAnimations];
    
    appDelegate.strMenuSelected = @"SPORT";
	showAlert = YES;
    ...
    viewBGColored.backgroundColor = [UIColor colorWithRed:247/255.f green:174/255.f blue:11/255.f alpha:1.0]; 
...
- (void)Food:(UIButton *)sender {
	...
	arrow.image = arw;
     */
    imgMovable.frame = CGRectMake(-61, 0, 575, 72);
    
    [UIView commitAnimations];
    
    appDelegate.strMenuSelected = @"FOOD";
    ...
    viewBGColored.backgroundColor = [UIColor colorWithRed:102/255.f green:179/255.f blue:88/255.f alpha:1.0]; 
...
- (void)ShowAll:(UIButton *)sender {
	...
	arrow.image = arw;
     */
    imgMovable.frame = CGRectMake(-253, 0, 575, 72);
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	if ([UIApplication sharedApplication].networkActivityIndicatorVisible == NO) {
        ...
        // coming soon
				...
				if ([listOfEvents count] == 0 && [wthot count] != 0) {
					...
					if (showAlert==YES) {
						/*	UIAlertView *alertview = [[UIAlertView alloc] initWithTitle:nil
                         ...
                         otherButtonTitles:nil];	
                         [alertview show];
                         [alertview release];*/						showAlert = NO;
					}	
				}
			}
			else {
			...
			if ([UIApplication sharedApplication].networkActivityIndicatorVisible == NO) {
                NSLog(@"List of Events : %i", listOfEvents.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	int n = appRecord.intr;
		...
		if (cell == nil)
		{
            ...
            eventImage.tag = indexPath.row + 5000;
			...
			accesory = [[UIImageView alloc] init];
            ...
            eventImage = (UIImageView *)[cell.contentView viewWithTag:indexPath.row + 5000];
		...
		if (nodeCount > 0)
		{
			...
			appRecord.eventTitle = [appRecord.eventTitle stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
			...
			else
			{
               ...
               NSLog(@"event Image when scroll : %f",appRecord.eventImage.size.width);
                NSLog(@"event Image when scroll : %f",appRecord.eventImage.size.height);
                NSLog(@"event Image when scroll at index : %d IMAGE : %@",indexPath.row,appRecord.imageURLString);
                //cell.imageView.image = [self maskImage:appRecord.eventImage];//[self maskImage:appRecord.eventImage withMask:circle];
                CALayer *mask = [CALayer layer];
                mask.contents = (id)[[UIImage imageNamed:@"Thumb_mask.png"] CGImage];
                mask.frame = CGRectMake(0, 0, 40, 40);
                eventImage.layer.mask = mask;
                [eventImage.layer setMasksToBounds:YES];
                eventImage.image = appRecord.eventImage;
			}
			accesory.frame = CGRectMake(286.6, 20.2, 22.8, 22.8);
            eventImage.frame = CGRectMake(10, 10, 40, 40);
			
			int j = appRecord.img;
            
			if (j == 1){
				//UIImage *indicatorImage2;
                ...
                if ([appRecord.category isEqualToString:@"Music"]) {
                    UIImage *indicatorImage = [UIImage imageNamed:@"icon_arrow_default.png"];
...
- (void)appImageDidLoad:(NSIndexPath *)indexPath
{
    ...
    if (iconDownloader != nil)
    {
		...
		[arts setUserInteractionEnabled:YES];
		[tool setUserInteractionEnabled:YES]; 
        NSLog(@"event Image : %f",iconDownloader.appRecord.eventImage.size.width);
        NSLog(@"event Image : %f",iconDownloader.appRecord.eventImage.size.height);
        
        //cell.imageView.image =[self maskImage:iconDownloader.appRecord.eventImage];
        UIImageView *eventImageLocal = (UIImageView *)[cell.contentView viewWithTag:indexPath.row + 5000];
        CALayer *mask = [CALayer layer];
        mask.contents = (id)[[UIImage imageNamed:@"Thumb_mask.png"] CGImage];
        mask.frame = CGRectMake(0, 0, 40, 40);
        eventImageLocal.layer.mask = mask;
        [eventImageLocal.layer setMasksToBounds:YES];
        eventImageLocal.image = iconDownloader.appRecord.eventImage;

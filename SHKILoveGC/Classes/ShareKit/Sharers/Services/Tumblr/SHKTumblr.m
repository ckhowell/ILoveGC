...
- (void)sendFinished:(SHKRequest *)aRequest{
	if (!aRequest.success) {
		if (aRequest.response.statusCode == 403) {
			[self sendDidFailWithError:[SHK2 error:SHKLocalizedString(@"Invalid email or password.")] shouldRelogin:YES];
			return;
		}
		...
		[self sendDidFailWithError:[SHK2 error:SHKLocalizedString(@"There was a sending your post to Tumblr.")]];
		return;
	}
